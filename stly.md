...
- Use the same top menu on the catalog and every game page: Catalog, Крестики-нолики, Змейка, Pong, Breakout, Сапер, Space Invaders, 15 Puzzle, Memory Match, Simon, Frogger, Hangman, Asteroids, Tetris, Whac-A-Mole, Lunar Lander, Connect Four, Centipede, Kaboom!, Sokoban, Missile Command, Pac-Man
...
- Sokoban, Pac-Man, and Missile Command use generated SVG assets while keeping the same compact board/status/button pattern as the other catalog games.
- Connect Four, Centipede, Kaboom!, Sokoban, Missile Command, and Pac-Man use the same compact board/status/button pattern as the other catalog games.
- Pac-Man uses the generated `assets/pac-man-maze.svg` asset as a subtle canvas backdrop.
